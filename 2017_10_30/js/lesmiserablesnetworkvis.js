function Network() {
  ...
  function setupData(data) {
    ...
    var grafico = Grafico(data);
    //initialize circle radius scale
    countExtent = d3.extent(data.nodes, d => grafico[d.id]);

    //countExtent = d3.extent(data.nodes, function(d) {
    //  return d.playcount;
    //});
    //Cuidado com a escala, aqui uma escala linear será mais apropriada
    ...
    data.nodes.forEach(function(n) {
      ...
      n.degree = grafico[n.id];
      // add radius to the node so we can use it later
      n.radius = circleRadius(grafico[n.id]);
